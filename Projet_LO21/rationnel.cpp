...
{
    genre = a.genre;
    numerateur = new Entier(a.getNum()->getChaine());
    denominateur = new Entier(a.getDen()->getChaine());
}

std::string Rationnel::getChaine() const
{
    stringstream ss;
    ss << numerateur->getChaine() << "/" << denominateur->getChaine();

    return ss.str();
}

void Rationnel::construireConstante(const std::string& val)
{
...
Rationnel::Rationnel(const std::string& val, const Type& T1, const Type& T2) : numerateur(new Entier("0")), denominateur(new Entier("0"))
{
    ...
    else if(T1 == complexe)
    {
        ...
        numerateur->setVal(temp.getValNum());
        denominateur->setVal(temp.getValDen());
        genre = entier;
    }
}
